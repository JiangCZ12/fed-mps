...
#matplotlib.use('Agg')
def check_rele(model, old_model, last_updates):
    sign_sum = 0
    sign_size = 0
    rel_threshold = 0.5
    model_para_list = []
...
class LocalUpdate(object):
    ...
    def update_weights(self, net):
        ...
        for param, prev_param in zip(net.parameters(), net1.parameters()):
            update_direction = torch.sign(param - prev_param)
            param.data = torch.where(update_direction == 1, param, torch.tensor(0.))
